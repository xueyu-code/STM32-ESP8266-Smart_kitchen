...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagmq5_threshold)
    {
        info->event[info->num] = EVENT_mq5_threshold;
        info->num++;
        dataPoints->valuemq5_threshold = gizX2Y(mq5_threshold_RATIO,  mq5_threshold_ADDITION, exchangeBytes(issuedData->attrVals.valuemq5_threshold));
    }
        
    if(0x01 == issuedData->attrFlags.flagmq9_threshold)
    {
        info->event[info->num] = EVENT_mq9_threshold;
        info->num++;
        dataPoints->valuemq9_threshold = gizX2Y(mq9_threshold_RATIO,  mq9_threshold_ADDITION, exchangeBytes(issuedData->attrVals.valuemq9_threshold));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valuemq5_threshold != cur->valuemq5_threshold)
    {
        GIZWITS_LOG("valuemq5_threshold Changed\n");
        ret = 1;
    }
    if(last->valuemq9_threshold != cur->valuemq9_threshold)
    {
        GIZWITS_LOG("valuemq9_threshold Changed\n");
        ret = 1;
    }
    if(last->valueHeat_flag != cur->valueHeat_flag)
    {
        GIZWITS_LOG("valueHeat_flag Changed\n");
        ret = 1;
    }
    if(last->valueMQ2_flag != cur->valueMQ2_flag)
    {
        GIZWITS_LOG("valueMQ2_flag Changed\n");
        ret = 1;
    }
    if(last->valueMQ5_flag != cur->valueMQ5_flag)
    {
        GIZWITS_LOG("valueMQ5_flag Changed\n");
        ret = 1;
    }
    if(last->valueMQ9_flag != cur->valueMQ9_flag)
    {
        GIZWITS_LOG("valueMQ9_flag Changed\n");
        ret = 1;
    }

    if(last->valueHeat != cur->valueHeat)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueHeat Changed\n");
            ret = 1;
        }
    }
    ...
    if(last->valueMQ9 != cur->valueMQ9)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueMQ9 Changed\n");
            ret = 1;
        }
    }
    if(last->valueMQ5 != cur->valueMQ5)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueMQ5 Changed\n");
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizStandardCompressValue(Heat_flag_BYTEOFFSET,Heat_flag_BITOFFSET,Heat_flag_LEN,(uint8_t *)devStatusPtr,dataPoints->valueHeat_flag);
    gizStandardCompressValue(MQ2_flag_BYTEOFFSET,MQ2_flag_BITOFFSET,MQ2_flag_LEN,(uint8_t *)devStatusPtr,dataPoints->valueMQ2_flag);
    gizStandardCompressValue(MQ5_flag_BYTEOFFSET,MQ5_flag_BITOFFSET,MQ5_flag_LEN,(uint8_t *)devStatusPtr,dataPoints->valueMQ5_flag);
    gizStandardCompressValue(MQ9_flag_BYTEOFFSET,MQ9_flag_BITOFFSET,MQ9_flag_LEN,(uint8_t *)devStatusPtr,dataPoints->valueMQ9_flag);
    ...
    devStatusPtr->valuemq5_threshold = exchangeBytes(gizY2X(mq5_threshold_RATIO,  mq5_threshold_ADDITION, dataPoints->valuemq5_threshold)); 
    devStatusPtr->valuemq9_threshold = exchangeBytes(gizY2X(mq9_threshold_RATIO,  mq9_threshold_ADDITION, dataPoints->valuemq9_threshold)); 
    devStatusPtr->valueHeat = exchangeBytes(gizY2X(Heat_RATIO,  Heat_ADDITION, dataPoints->valueHeat)); 
    devStatusPtr->valueMQ2 = exchangeBytes(gizY2X(MQ2_RATIO,  MQ2_ADDITION, dataPoints->valueMQ2)); 
    devStatusPtr->valueMQ9 = exchangeBytes(gizY2X(MQ9_RATIO,  MQ9_ADDITION, dataPoints->valueMQ9)); 
    devStatusPtr->valueMQ5 = exchangeBytes(gizY2X(MQ5_RATIO,  MQ5_ADDITION, dataPoints->valueMQ5)); 
